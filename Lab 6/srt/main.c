...
void* pixeler(void* args) {
    ...
    /* for every pixel */
    for( int px= *(int*)args; px<width; px += nthreads )
    {
...
main( int argc, char **argv )
{
    ...
    for (int i = 0; i < nthreads; i++) {
        thread_ids[i] = i;
        if(pthread_create(threads + i, NULL, pixeler, (void *) (thread_ids+i))) {
    ...
    for(int t = 0; t < nthreads; t++){
        if(pthread_join(threads[t], NULL)){
            fprintf(stderr, "Error: cannot free thread\n");
            exit(1);
        }
    }
